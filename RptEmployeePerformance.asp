...
Set adCmd = GetAdoCmd("spRptEmployeePerformance")
...
Sub window_onload
    ...
    cmdExport1.style.left = 100
    cmdExport2.style.left = 100
...
For Each oElement In dctElements
    adRsFull.Filter = "prgID=" & Parse(oElement,"^",1) & " AND elmTypeID=" & Parse(oElement,"^",2) & " AND elmID=" & Parse(oElement,"^",3)
    adRsFull.MoveFirst
    ...
    For Each oFactor In dctFactors
        If Parse(oFactor,"^",1) & "^" & Parse(oFactor,"^",2) & "^" & Parse(oFactor,"^",3) = oElement Then
            ...
            End If
            'oReport.WriteLine oFactor & " -- " & dctFactors(oFactor)
            adRsFactors.Filter = "fctID=" & Parse(oFactor,"^",4)
            Call StartTable()
            Call StartTableBody()
            mintRowID = mintRowID + 1
            Call AddTableColumn(10,"&nbsp;",False,"TableRow")
            If intHoldTabID = 2 Then
                Call AddTableColumn(320,adRsFactors.Fields("fctShortName").value,False,"TableRow")
                Call AddTableColumn(90,Parse(dctFactors(oFactor),"^",3),False,"TableRow^text-align:center;^")
            Else
                Call AddTableColumn(410,adRsFactors.Fields("fctShortName").value,False,"TableRow")
            End If
...
Class clsReport
    ...
    Private Sub AddFactor(strKey, intFactorStatus)
        ...
        strNewItem = ""
        For intI = 1 To 3
            If CInt(intI) = CInt(intFactorStatus) - 21 Then
    ...
    Private Sub AddElement(strKey, intElementStatus)
        ...
        End If
        strNewItem = ""
        Select Case CInt(intElementStatus)
    ...
    Public Sub ProcessRecordset()
        ...
        Do While Not adRs.EOF
            If adRs.Fields("rveTypeID").value = 3 And blnLastDI = True Then
                ...
                blnLastDI = False
            End If
            If adRs.Fields("rvfFactorID").value > 0 Then
                Call AddFactor(adRs.Fields("rveProgramID").value & "^" & adRs.Fields("rveTypeID").value & "^" & adRs.Fields("rveElementID").value & "^" & adRs.Fields("rvfFactorID").value, adRs.Fields("rvfStatusID").value)
            End If
            Select Case adRs.Fields("rveTypeID").value
                Case 1
                    ...
                    If Not dctAIKeys.Exists(adRs.Fields("rveReviewID").value & "^" & adRs.Fields("rveProgramID").value & "^" & adRs.Fields("rveTypeID").value & "^" & adRs.Fields("rveElementID").value) Then
                        Call AddElement(adRs.Fields("rveProgramID").value & "^" & adRs.Fields("rveTypeID").value & "^" & adRs.Fields("rveElementID").value, adRs.Fields("rveStatusID").value)
                        dctAIKeys.Add adRs.Fields("rveReviewID").value & "^" & adRs.Fields("rveProgramID").value & "^" & adRs.Fields("rveTypeID").value & "^" & adRs.Fields("rveElementID").value, ""
                    End If
                Case 2
                    'For Data Integrity, element status is determined from the factors
                    If aDIElmStatus(1) & "^" & aDIElmStatus(2) & "^" & aDIElmStatus(3) <> adRs.Fields("rveProgramID").value & "^" & adRs.Fields("rveElementID").value & "^" & adRs.Fields("rveReviewID").value Then
                        'Screen changed, add Element status to object
                        If aDIElmStatus(1) > 0 Then
                         'rESPONSE.Write  aDIElmStatus(1) & "^" & aDIElmStatus(2) & "^" & aDIElmStatus(3) & "  ==  " & adRs.Fields("rveProgramID").value & "^" & adRs.Fields("rveElementID").value & "^" & adRs.Fields("rveReviewID").value & "<br>"
                         '
                            Call AddElement(aDIElmStatus(1) & "^2^" & aDIElmStatus(2), aDIElmStatus(4))
                        End If                   
                        aDIElmStatus(1) = adRs.Fields("rveProgramID").value
                        aDIElmStatus(2) = adRs.Fields("rveElementID").value
                        aDIElmStatus(3) = adRs.Fields("rveReviewID").value
                        aDIElmStatus(4) = 0
                    End If
                    aDIElmStatus(4) = RollingScreenStatus(aDIElmStatus(4), adRs.Fields("rvfStatusID").value)
                    blnLastDI = True
                Case 3
                    Call AddElement(adRs.Fields("rveProgramID").value & "^" & adRs.Fields("rveTypeID").value & "^" & adRs.Fields("rveElementID").value, adRs.Fields("rveStatusID").value)
            End Select

...
If Parse(strParmList,"^",2) = "True" Then
    blnUserAdmin = 1
Else
    blnUserAdmin = 0
End If
If Parse(strParmList,"^",3) = "True" Then
    blnUserQA = 1
Else
    blnUserQA = 0
End If
...
If strLoad = "Y" Then
    ...
    Set adCmd = GetAdoCmd("spReviewFind")
	    AddParmIn adCmd, "@AliasID", adInteger, 20, mlngAliasID
...
Sub PrintList()
    ...
    strDetails = strDetails & "&A16=<%=lngReviewClassID%>"
...
<BODY id=PageBody bottomMargin=5 leftMargin=5 topMargin=5 rightMargin=5 style="cursor:wait">
    <DIV id=lstResults class=TableDivArea title="Results of Find"
        style="LEFT:0; WIDTH:735; TOP:0; HEIGHT:315"
        tabIndex=-1>
        <%
        Dim intResultCnt
        Dim mintTblCols

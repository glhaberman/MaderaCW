...
'---------------------------------------------------------------------------'
'     Name: ReviewerSum.asp                                                 '
'  Purpose: Generates the Reviewer Case Status report based on the criteria '
...
Dim strBackColor
Dim strFontCOlor
Dim mstrTargetedReviewName
Dim strName
Dim strRangeHeading, mstrRevCaseCountReportDates
%>
<!--#include file="IncCnn.asp"-->
<!--#include file="IncValidUser.asp"-->
<!--#include file="IncDrillDownSvr.asp"-->
<%
maStaffParms(1) = ReqIsBlank("Reviewer")
mstrRevCaseCountReportDates = GetAppSetting("RevCaseCountReportDates")
mstrTargetedReviewName = GetAppSetting("TargetedReviewName")
If mstrTargetedReviewName = "" Then mstrTargetedReviewName = "Targeted"
mstrPageTitle = gstrLocationName & " " & gstrOrgName & " " & gstrAppName
Set adCmd = GetAdoCmd("spRptReviewerCount")
    ...
    AddParmIn adCmd, "@Worker", adVarChar, 50, Null
...
Sub ColClickEvent(intColID, intRowID)
    Call DrillDownColClickEvent("spRptReviewerCount", intColID, intRowID, True, 0)
End Sub 
-->
</SCRIPT>
...
<!--#include file="IncRptHeader.asp"-->
    <DIV id=PageFrame
        ...
        Do While Not adRs.EOF
            ...
            'Reset values
            For intI = icRVW To intReqLevel + 1 Step -1
            ...
            For intI = icRVW To icWRK Step -1
            ...
            'oCounts.TotalTarg(icWRK) = adRs.Fields("TargettedCnt").Value
            'oCounts.TotalFull(icWRK) = adRs.Fields("FullCnt").Value
            oCounts.TotalCount(icWRK) = adRs.Fields("TotalCases").Value 
            adRs.MoveNext
            ...
            If aDoTotals(icWRK) Then
                ...
                Call WriteTotals(icWRK, "ManagementText", strWkrColor, "#000000" )
    ...
    If ReqForm("Director") = "" And ReqForm("ProgramManager") = "" And ReqForm("Reviewer") = "" Then
        Response.Write "<BR style=""FONT-SIZE:16"">"
...
Sub WriteTotals(intWhich, strClass, strBackColor, strFontColor)
    Call oCounts.NewRow()
    'response.Write "<br>intWhich=" & intWhich & "<br>"
    Call WriteNames(intWhich)
    Call WriteColumn(1,strClass,oCounts.TotalCount(intWhich),360,strBackColor,"",100,0)
End Sub
Sub WriteColumnHeaders(strNameTitle)
    Response.Write "<SPAN id=lblWorker class=ColumnHeading "
    Response.Write "style=""WIDTH:640;height:20; LEFT:10;BORDER-BOTTOM-STYLE:none; BACKGROUND:" & strRvwColor & """>"
    Response.Write "</SPAN>"    
    
    Response.Write "<SPAN id=lblFullCnt class=ColumnHeading "
    Response.Write "style=""WIDTH:100; LEFT:360;BORDER-BOTTOM-STYLE:none; BACKGROUND:" & strRvwColor & """>"
    Response.Write "Total</SPAN>"

    Response.Write "<BR>"
    
    Response.Write "<SPAN id=lblWorker class=ColumnHeading "
    Response.Write "style=""WIDTH:640; LEFT:10;BORDER-TOP-STYLE:none; BACKGROUND:" & strRvwColor & """>"
    Response.Write "</SPAN>"
    
    Response.Write "<SPAN id=lblWorker class=ColumnHeading "
    Response.Write "style=""WIDTH:340; LEFT:10;BORDER-TOP-STYLE:none; BACKGROUND:" & strRvwColor & """>"
    Response.Write strNameTitle & "&nbsp Name</SPAN>"
    
    Response.Write "<SPAN id=lblFullCnt class=ColumnHeading "
    Response.Write "style=""WIDTH:100; LEFT:360;BORDER-TOP-STYLE:none; BACKGROUND:" & strRvwColor & """>"
...
Class clsCounters
    ...
    Private aTotalCount(6)     'Total Review Count

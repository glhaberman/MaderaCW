...
<!--#include file="IncRptHeader.asp"-->
    <DIV id=PageFrame
		...
		strWkrColor = "#ffffff"
		strSupColor = "#FFEFD5"
		If intReqLevel = icSUP And Not adRs.EOF Then
		...
		Do While Not adRs.EOF
            ...
            'Reset values
            For intI = icSUP To intReqLevel + 1 Step -1
            ...
            For intI = icSUP To icWRK Step -1
				If oCounts.Changed(intI) AND oCounts.ShowLevel(intI) Then
					If aShowLevel(intI) Then
						...
						Response.Write "style=""WIDTH:270; BackGround:" & strBackColor & """>"
			...
			For intI = icSUP To icSUP
				...
				If aDoTotals(intI) Then
				    If intTempReqLevel < intReqLevel And intI = intReqLevel Then
				        ' If on the level that was originally the lowest level (intReqLevel) AND
				        ' the report had to drop down an additional level or more (intTempReqLevel)
				        ' because levels were skipped on the review (a worker is 
				        ' directly under a region, with no sup), DO NOT PRINT totals.  For example, if
				        ' Worker 1, Worker 2 and Supervisor 1 were all directly under Region 1, the report data would look like:
				        '  Region 1         *Region 1       Worker 1
				        '  Region 1         *Region 1       Worker 2
				        '  Region 1         Supervisor 1    Worker 3
				        '  Region 1         Supervisor 1    Worker 4
				        ' Totals will print for Worker 1 and Worker 2 because they are directly under Region 1.  This change
				        ' skips the total printing for *Region 1 a the supervisor level.  The report will look like
				        '  Division 1
				        '  Region 1
				        '  Worker 1             999
				        '  Worker 2             999
				    ...
				    Else
					    ...
					    If oCounts.ShowLevel(intI - 1) Then
						    ...
						    Response.Write "<span id=lblSup class=" & strClass & " "
						    Response.Write "style=""BACKGROUND:" & strBackColor & "; TEXT-ALIGN:LEFT "">"
			...
			Next
	    Loop
	    If aDoTotals(icFIN) And oCounts.FormName(icWRK) = "" And oCounts.FormName(icSUP) = "" And oCounts.FormName(icMGR) = "" And oCounts.FormName(icDIR) = "" Then
		    Response.Write "<BR style=""FONT-SIZE:16"">"
...
Sub WriteTotals(intWhich, strClass, strBackColor, strFontColor)
	...
	Response.Write "<SPAN id=lblErrorPercent class=" & strClass & " " & vbCrLf
	Response.Write "style=""WIDTH:90; LEFT:400; BACKGROUND:" & strBackColor & "; TEXT-ALIGN:right"">" & vbCrLf
	Response.Write oCounts.PercentError(intWhich) & "</SPAN>" & vbCrLf
	
	Response.Write "<SPAN id=lblPMPercentCorrect1 class=" & strClass & " " & vbCrLf
	Response.Write "style=""WIDTH:90; LEFT:560; BACKGROUND:" & strBackColor & ";TEXT-ALIGN:right"">" & vbCrLf
	Response.Write oCounts.PercentCorrect(intWhich) & "</SPAN>" & vbCrLf
End Sub

...
Set adCmd = GetAdoCmd("spRptArcCaseDetail")
    ...
    AddParmIn adCmd, "@ReviewType", adVarChar, 2000, ReqIsBlank("ReviewTypeText")
    AddParmIn adCmd, "@ReviewClass", adVarChar, 2000, ReqIsBlank("ReviewClassText")
...
Sub window_onload
    ...
    If "<%=mstrShowDetail%>" = "N" Then
        If "<%=ReqForm("ProgramID")%>" = "0" Then
            lblAppTitle.innerText = "Case Review Listing: All Functions"
            lblAppTitle.style.fontweight = "bold"
        End If
    Else
        If "<%=ReqForm("ProgramID")%>" = "0" Then
            lblAppTitle.innerText = "Worker Case Review Listing: All Functions"
            lblAppTitle.style.fontweight = "bold"
        Else
...
Sub ColClickEvent(intColID, intRowID)
    ...
    strReturnValue = window.showModalDialog("ArcPrintReview.asp?UserID=<%=gstrUserID%>&ReviewID=" & document.all("hidRowInfo" & intRowID).value, , "dialogWidth:710px;dialogHeight:520px;scrollbars:no;center:yes;border:thin;help:no;status:no")
End Sub
...
Dim mstrHidden, intPrgCount, strValue, strValue2
...
Do While Not adRs.EOF
    If strHoldWorker <> adRs.Fields("Worker").value Then
        If mintRowID > 1 Then
            Response.Write "<BR>"
        End If
    ...
    If mstrShowDetail = "Y" Then
        ...
        adRsElms.Filter = "rveReviewID=" & adRs.Fields("rvwID").value & " And FunctionName='" & adRs.Fields("ProgramName").value & "'"
        'adRsElms.Filter = "FunctionName='" & adRs.Fields("ProgramName").value & "'"
        If adRsElms.RecordCount > 0 Then
            Do While Not adRsElms.Eof
                If adRsElms.Fields("rveType").value <> strHoldTab Then
                    If strHoldTab <> "" Then
                        Call SingleLineTable("&nbsp;", "GroupHeader", 20)
                    End If
                    strHoldTab = adRsElms.Fields("rveType").value
                    Call SingleLineTable(strHoldTab, "GroupHeader", 20)
                    Select Case adRsElms.Fields("rveType").value
                        Case "Action Integrity", "Information Gathering"
                            Call PrintAIHeaders()
                        Case "Data Integrity"
                            strHoldScreen = ""
                    End Select
                ...
                Select Case adRsElms.Fields("rveType").value
                    ...
                    Case "Data Integrity"
                        ...
                        adRsFacs.Filter = "rvfReviewID=" & adRs.Fields("rvwID").value & _
                                          " And FunctionName=" & adRs.Fields("ProgramName").value & _
                                          " And ScreenName=" & adRsElms.Fields("ElementName").value
                        If adRsFacs.RecordCount > 0 Then
                            Do While Not adRsFacs.Eof
                                ...
                                mintRowID = mintRowID + 1
                                Call AddTableColumn(30,"&nbsp;",False,"TableRow")
                                Call AddTableColumn(200,adRsFacs.Fields("FactorName").value,False,"TableRow")
                                Call AddTableColumn(250,adRsFacs.Fields("FactorStatus").value,False,"TableRow")
                                Call EndTableBody()
                                Call EndTable()
                                adRsFacs.MoveNext
                            Loop
            ...
            Loop
        End If
        adRsAuds.Filter = "rvaReviewID=" & adRs.Fields("rvwID").value
        adRsAuds.Sort = "rvaDateOfAction"
        intI = 0
        If adRsAuds.RecordCount > 0 Then
            ...
            Do While Not adRsAuds.Eof
                For intJ = 1 To 100
                    ...
                    intI = intI + 1
                    maAuditRecords(intI,0) = FormatDateTime(adRsAuds.Fields("rvaDateOfAction").value,2) & " " & FormatDateTime(adRsAuds.Fields("rvaDateOfAction").value,3)
                    maAuditRecords(intI,1) = adRsAuds.Fields("rvaUserID").value
...
Sub PrintDIHeaders()
    ...
    Call StartTableBody()
    Call AddTableColumn(30,"&nbsp;", False, "TableRow")
    Call AddTableColumn(200,"Field",False,"RowHeader")
    Call AddTableColumn(250,"Status",False,"RowHeader")
    Call EndTableBody()
    Call EndTable()
End Sub
